...
final class SeanceController extends AbstractController {
    ...
    {   
        ...
        if($seance->getSportifs()->contains($user)){
            return $this->json(['error' => 'Sportif déjà inscrit à la séance'], JsonResponse::HTTP_BAD_REQUEST);
        }
        ...
        foreach($seances as $s){
            ...
            if($dateDebut > new DateTime()){
                $cptSeances++;
                if($cptSeances >= 3){
                    return $this->json(['error' => 'Sportif a déjà 3 séances à venir'], JsonResponse::HTTP_BAD_REQUEST);
                }
            }
            ...
            if($seance->getDateHeure() >= $dateDebut && $seance->getDateHeure() <= $dateFin){
                return $this->json(['error' => 'Sportif déjà inscrit à une séance à cette heure'], JsonResponse::HTTP_BAD_REQUEST);
            }
        }
